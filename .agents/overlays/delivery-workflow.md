...
- `make validate` is the native changed-surface entry and delegates to
  ...
  baseline and artifact gate, watching the ignored `.xcode-build/Build/`
  output; a root created only for the run is cleaned before the wrapper
  returns. The separate `make test` gate uses `.xcode-build-tests/`.
